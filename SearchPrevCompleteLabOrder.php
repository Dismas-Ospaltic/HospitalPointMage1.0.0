...
$select_Hist = mysqli_query($conn, "SELECT * FROM laboratory_test WHERE (hospital_patient_no LIKE '%{$searchTerm}%' OR patient_name LIKE '%{$searchTerm}%' OR sent_by LIKE '%{$searchTerm}%' OR sample_no LIKE '%{$searchTerm}%' OR visit_id LIKE '%{$searchTerm}%' OR return_date LIKE '%{$searchTerm}%' OR lab_tech LIKE '%{$searchTerm}%') AND status='tested' ORDER BY id DESC");

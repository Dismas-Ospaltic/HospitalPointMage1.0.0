...
$select_Incoming = mysqli_query($conn, "SELECT * FROM laboratory_test WHERE (hospital_patient_no LIKE '%{$searchTerm}%' OR patient_name LIKE '%{$searchTerm}%' OR sent_by LIKE '%{$searchTerm}%') AND status='tested'  AND request_date='{$current_date}' ORDER BY id DESC");

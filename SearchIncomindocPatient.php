...
$select_patient_Inco_data = mysqli_query($conn, "SELECT * FROM patient_sub_visit WHERE (hospital_patient_no LIKE '%{$searchTerm}%' OR pateint_name LIKE '%{$searchTerm}%'
 OR urgency LIKE '%{$searchTerm}%' OR visit_reason LIKE '%{$searchTerm}%' OR odp_idp_no LIKE '%{$searchTerm}%') AND status='not_tend' ORDER BY id ASC");

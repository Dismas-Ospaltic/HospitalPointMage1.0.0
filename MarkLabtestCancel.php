...
if(isset($_GET["HSPN"]) && isset($_GET["V_ID"])){
    ...
    $update_status = mysqli_query($conn, "UPDATE laboratory_test SET status='canceled' WHERE hospital_patient_no='{$HPT_NO}' AND visit_id='{$VISIT_ID}' AND status='not_tested'");
if($update_status){
echo "success";
}else{
echo "failed";

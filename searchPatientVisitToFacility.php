...
if(isset($_GET["HSPN"]) && isset($_GET["ODPIDP"])){
    ...
    $select_visits = mysqli_query($conn, "SELECT visit_date,visit_time,discharge_date,visit_id,visit_reason,tend_by,hospital_patient_no,odp_idp_no FROM patient_sub_visit WHERE (visit LIKE '%{$searchTerm}%' OR visit_date LIKE '%{$searchTerm}%' OR tend_by LIKE '%{$searchTerm}%' 
    OR department LIKE '%{$searchTerm}%' OR visit_reason LIKE '%{$searchTerm}%' OR visit_id LIKE '%{$searchTerm}%') AND (hospital_patient_no ='{$hospital_unique}' OR odp_idp_no ='{$DP_NO}') ORDER BY id DESC");
